...
namespace glz
{
   namespace detail
   {
      ...
      struct read<cbor>
      {
         ...
         {
            from_cbor<std::decay_t<T>>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx),
      ...
      struct from_cbor<T>
      {
         ...
         GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end)
         {
            ...
            from_cbor<V>::template op<Opts>(get_member(value, meta_wrapper_v<T>), std::forward<Ctx>(ctx),
                                              std::forward<It0>(it), std::forward<It1>(end));
         }
      };
   ...
   GLZ_ALWAYS_INLINE parse_error read_cbor(T&& value, Buffer&& buffer) noexcept
   {
      return read<opts{.format = binary}>(value, std::forward<Buffer>(buffer));
   }

   template <class T, class Buffer>
   GLZ_ALWAYS_INLINE expected<T, parse_error> read_cbor(Buffer&& buffer) noexcept
   {
      T value{};
   ...
   template <class T>
   GLZ_ALWAYS_INLINE parse_error read_file_cbor(T& value, const sv file_name) noexcept
   {
      context ctx{};
